    ...
    return (
      <section aria-labelledby="success-title" className="grid min-w-0 gap-6">
        <div className="min-w-0 rounded-md border border-emerald-200 bg-emerald-50 p-5">
          <h2
            id="success-title"
            className="wrap-break-word text-2xl font-bold text-emerald-950"
          >
            신청이 완료되었습니다.
          </h2>
          <dl className="mt-4 grid min-w-0 gap-2 text-sm text-emerald-950">
            <div className="min-w-0">
              ...
              <dd className="wrap-break-word">{result.enrollmentId}</dd>
            </div>
            <div className="min-w-0">
            ...
            <div className="min-w-0">
              ...
              <dd className="wrap-break-word">{formatDateTime(result.enrolledAt)}</dd>
            </div>
          </dl>
        </div>
        <SummarySections
...
function SummarySections({
  ...
  return (
    <div className="grid min-w-0 gap-4">
      <section
        className="min-w-0 wrap-break-word rounded-md border border-slate-200 bg-white p-4"
      ...
      >
        ...
        <p className="mt-3 min-w-0 wrap-break-word text-sm text-slate-700">
          {selectedCourse?.title ?? '선택한 강의 없음'}
        </p>
      </section>

      <section className="min-w-0 wrap-break-word rounded-md border border-slate-200 bg-white p-4">
        ...
        <dl className="mt-3 grid min-w-0 gap-2 text-sm text-slate-700">
          <div className="min-w-0">
            ...
            <dd className="wrap-break-word">{formState.applicant.name}</dd>
          </div>
          <div className="min-w-0">
            <dt className="font-semibold text-slate-950">이메일</dt>
            <dd className="wrap-break-word">{formState.applicant.email}</dd>
          </div>
          <div className="min-w-0">
            <dt className="font-semibold text-slate-950">전화번호</dt>
            <dd className="wrap-break-word">{formState.applicant.phone}</dd>
          </div>
        </dl>
      </section>
        ...
        <section className="min-w-0 wrap-break-word rounded-md border border-slate-200 bg-white p-4">
          ...
          <dl className="mt-3 grid min-w-0 gap-2 text-sm text-slate-700">
            <div className="min-w-0">
              ...
              <dd className="wrap-break-word">{formState.group.organizationName}</dd>
            </div>
            <div className="min-w-0">
              <dt className="font-semibold text-slate-950">신청 인원수</dt>
              <dd className="wrap-break-word">{formState.group.headCount}명</dd>
            </div>
            <div className="min-w-0">
              <dt className="font-semibold text-slate-950">담당자 연락처</dt>
              <dd className="wrap-break-word">{formState.group.contactPerson}</dd>
            </div>
          </dl>
